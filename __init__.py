...
def create_app(test_config=None):
    ...
    def log_in():
        if request.method == 'POST':
            ...
            if not user or not check_passowrd_hash(user.password, password):
    ...
    def note_create():
        if request.method == 'POST':
            ...
            if error is None:
                ...
                db.session.add(note)
                db.session.commit()
                flash(f"Successfully created a note: {title}", 'success')
    ...
    def note_delete(note_id)
        ...
        db.session.commit()
        flash(f"Successfull deleted note: '{note.title}'", 'success')
        return redirect(url_for('note_index'))
